import { LayoutEngine, LayoutNode, Direction, TileNode, LocalStorageAdapter, getDropAction, DropAction, findNode, HttpPersistenceAdapter } from '@ug-layout/core';
...
async function init() {
  ...
  engine.subscribe(() => {
    if (statusEl) {
      ...
      setTimeout(() => { if (statusEl.textContent === 'Saving...') { statusEl.textContent = 'Synced'; statusEl.style.color = '#4dff4d'; } }, 1500);
    }
    updateDOM();
  });
